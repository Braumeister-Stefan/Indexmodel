...
df = pd.read_csv(r'C:\Users\Gebruiker\Desktop\solactive AG\Book1.csv', sep=";")
...
stop_index = pd.to_datetime("2020-12-31")
...
placeholder = df["Date"].loc[df["Date"] == start_index].index[0]  # retrieve index position where the index should start

day_weight = weighting_frame.loc[weighting_frame["Date"] == start_index].reset_index(drop=True)  # weight for first date
...
# note "2" should be decided flexibly based on input when index starts
...
final_index = final_index.loc[final_index["Date"] >= start_index, :]
final_index = final_index.loc[final_index["Date"] <= stop_index, :]
